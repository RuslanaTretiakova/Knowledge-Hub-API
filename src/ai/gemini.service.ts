...
export class GeminiService {
  ...
  async embedTexts(texts: string[]): Promise<number[][]> {
    ...
    for (let i = 0; i < texts.length; i += EMBED_BATCH_SIZE) {
      ...
      const vectors = await this.embedBatchRequest(batch);
      out.push(...vectors);
    }
    return out;
  }

  private async embedBatchRequest(texts: string[]): Promise<number[][]> {
    const modelResource = `models/${this.embeddingModel}`;
    const url = `${this.baseUrl}/v1beta/${modelResource}:batchEmbedContents`;
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (!response.ok) {
          let detail = 'Gemini embedding request failed';
